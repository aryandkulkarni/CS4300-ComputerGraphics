...
void main()
{
    ...
    vec3 tangent = normalize(vec3(modelTransformMatrix * vec4(tangents, 0.0)));
    vec3 bitangent = normalize(vec3(modelTransformMatrix * vec4(bitangents, 0.0)));
    vec3 normal = normalize(vec3(modelTransformMatrix * vec4(normals, 0.0)));

    mat3 TBN_matrix = transpose(mat3(tangent, bitangent, normal));

    TangentFragPos = TBN_matrix * FragPos;
    TangentLightPos = TBN_matrix * lightPos;
    TangentViewPos  = TBN_matrix * viewPos;
